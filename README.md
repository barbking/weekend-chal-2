...
[x]client takes in two values in 2 input text fields and the type of math operation button,
  will be sent as object {x:, y:, type:}
[x]send to server vis post
[x]server receives object from client
[x]build out logic in module
[x]send calc back to client side app where it is displayed on DOM
